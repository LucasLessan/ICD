...
Original file is located at
    https://colab.research.google.com/drive/1ypWvxP_nAF5IoS4nFvdDN_sIe3wrDJMy
...
def high_delay(df):
    ...
    df['Tempo'] = np.abs(df['CRSArrTime'] - df['CRSDepTime'])

    media = df[['OriginAirportName', 'DestAirportName', 'Tempo']].groupby(['OriginAirportName', 'DestAirportName']).mean()

    media = media.sort_values(by='Tempo')

    return media.iloc[-1].name
...
segunda = np.array(df2[df2['DayOfWeek'] == 1]['DepDelay'])
terca = np.array(df2[df2['DayOfWeek'] == 2]['DepDelay'])
quarta = np.array(df2[df2['DayOfWeek'] == 3]['DepDelay'])
quinta = np.array(df2[df2['DayOfWeek'] == 4]['DepDelay'])
sexta = np.array(df2[df2['DayOfWeek'] == 5]['DepDelay'])
sabado = np.array(df2[df2['DayOfWeek'] == 6]['DepDelay'])
domingo = np.array(df2[df2['DayOfWeek'] == 7]['DepDelay'])

df3 = np.array([segunda, terca, quarta, quinta, sexta, sabado, domingo], dtype=object)
_ = plt.boxplot(df3, showfliers=False)
...
plt.scatter(clean_df(df)['DepDelay'], clean_df(df)['ArrDelay'], edgecolors='k')
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')
plt.show()