...
Original file is located at
    https://colab.research.google.com/drive/1lyTdJUEcDYKS5u4yQYOMAFr2KMp6bxLw
...
def percent(df1, df2, column, alpha):
    ...
    perc = percentil(alpha)
    
    a = np.round(np.percentile(values_diff, perc[0]), decimals=4)
    b = np.round(np.percentile(values_diff, perc[1]), decimals=4)
...
def t_obs(data):
    sal_pho = data[data['TEAM'] == 'Phoenix Suns']['SALARY'].mean()
    sal_cha = data[data['TEAM'] == 'Charlotte Hornets']['SALARY'].mean()
    
    return np.round(sal_pho - sal_cha, decimals=2)
...
def resposta(data):
    ...
    d1 = data[data['TEAM'] == 'Phoenix Suns']['SALARY']
    d2 = data[data['TEAM'] == 'Charlotte Hornets']['SALARY']
    ...
    for i in range(n):
        sample1 = d1.sample(size1, replace=True, random_state=i)
        sample2 = d2.sample(size2, replace=True, random_state=i*3)
...
# Gerando 5k dados de uma distribuição normal
np.random.seed(16)
...
def t_obs2(data):
    alt_gsw = data[data['team_abbreviation'] == 'GSW']['player_height'].mean()
    alt_cle = data[data['team_abbreviation'] == 'CLE']['player_height'].mean()
    
    return np.round(alt_gsw - alt_cle, decimals=2)
...
def critical_values(diffs):
    ...
    a = np.round(np.percentile(diffs, perc[0]), decimals=4)
    b = np.round(np.percentile(diffs, perc[1]), decimals=4)
...
def resposta2(diffs, t_obs):
    ...
    lim = critical_values(diffs)

    return not contem(lim, t_obs)
