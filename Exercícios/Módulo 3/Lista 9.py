...
Original file is located at
    https://colab.research.google.com/drive/1kCoDwPXcxIH0vMjYIiNOwEzKY7koAOjq
...
def minimize_batch(param, lambda_=0.0001, niter=1000000):
    ...
    for i in range(niter):
        grad = derivadas_xy(param)
        param_novos = param - lambda_ * grad
        if abs1(foo_f(param) - foo_f(param_novos)) < tolerance: 
            break
        param = param_novos
    return param
...
assert_equal(round(minimo, 4), -2.995)
assert_equal(round(x_min, 4), -3.9292)
assert_equal(round(y_min, 4), 1.9294)
...
def predict1(alpha, beta, x_i):
    return alpha + beta * x_i
    
def loss_um_ponto(x_i, y_i, alpha, beta):
    return (y_i - predict1(alpha, beta, x_i)) ** 2
...
## Observe a dica: voce deve somar os gradientes para cada parametro a cada ponto e só depois atualizar os valores com base no lambda
## Aqui, pode deixar com o número de iterações mesmo, sem critério de parada com tolerância
## gabarito:
...
#     for i in range(niter):
#         for j in range(n):
#             grad = gradient(x[j], y[j], param[0], param[1])
#             param_novos[0] = param[0] - lambda_ * grad[0]
#             param_novos[1] = param[1] - lambda_ * grad[1]
#             loss = loss_um_ponto(x[j], y[j], param[0], param[1])
#             loss_novo = loss_um_ponto(x[j], y[j], param_novos[0], param_novos[1])
#             if abs(loss_novo - loss) <= 0.00001: 
#                 break
#             param = param_novos
#     return param
...
#assert_equal(alpha, 1.8162991998465406e-14)
#assert_equal(beta, 0.9821058669740498)
...
## Aqui tem um macete pythonico!!
## Passando um vetor de valores para x e y, a função gradient já calcula ponto a ponto as derivadas
## Depois, basta pegar a soma das derivadas ponto a ponto e atualizar os parâmetros
## note
...
#     param = param.copy()
#     for i in range(niter):
#         grad = np.array(gradient(x, y, param[0], param[1])).mean(axis=1)
#         param_novos = param - lambda_ * grad
#         loss = loss_um_ponto(x, y, param[0], param[1]).mean()
#         loss_novo = loss_um_ponto(x, y, param_novos[0], param_novos[1]).mean()
#         if abs(loss_novo - loss) <= 0.00001: 
#             break
#         param = param_novos
...
[alpha, beta] = theta
#assert_equal(alpha, 1.8163015629435557e-14)
#assert_equal(beta, 0.9821058669740498)
...
def predict(x, param):
    return predict1(param[0], param[1], x)

preds = predict(df.values[:, 0], theta)
#assert_equal(preds.mean(), 1.7957145741885866e-14)
...
#assert_equal(soma_quadrados, 5.497550238548859)
...
assert_equal(rsquared, 0.964531933944846)
...
#assert_equal(age, 14.612822334220748)